...
public class ArvoreB {
	...
	private ElementoArvoreB pesquisa (ElementoArvoreB reg, Pagina ap) {
		if (ap == null){
	...
	private void insereNaPagina (Pagina ap, ElementoArvoreB reg, Pagina apDir) {
		int k = ap.n - 1;
	...
	private Pagina insere (ElementoArvoreB reg, Pagina ap, ElementoArvoreB[] regRetorno, boolean[] cresceu) {
	...
	private boolean reconstitui (Pagina apPag, Pagina apPai, int posPai) {
	  ...
	  private Pagina retira (ElementoArvoreB reg, Pagina ap, boolean[] diminuiu) {
		  if (ap == null) {
	  ...
	  public ElementoArvoreB pesquisa (ElementoArvoreB reg) {
		  return this.pesquisa (reg, this.raiz);
	  }

	  public void insere (ElementoArvoreB reg) {
		  ElementoArvoreB regRetorno[] = new ElementoArvoreB[1];
	  ...
	  public void retira (ElementoArvoreB reg) {
	  ...
	  public void geraArvore(String relacao) {
		  ...
		  while ((tupla = (Tupla) relacaoConstrucao.next()) != null) {
			  int idEditora = Integer.parseInt((String) tupla.getValoresRelacao().get(3));
			  Elemento elem = new Elemento(idEditora);
			  insere(elem);
			  imprime();
		  }		
